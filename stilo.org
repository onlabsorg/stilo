...
objects.
- [ ] Update and test the `stilo install` and `stilo.registerPlugin` hooks
- [ ] Update and test the `stilo uninstall` and `stilo.unregisterPlugin` hooks
...
- [ ] Update and test the `stilo.getStore` method
...
- [ ] Update and test `stilo.getCommands`, `stilo commands` and `stilo run`
